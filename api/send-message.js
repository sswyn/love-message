...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!token || !/^[0-9a-f]{48}$/.test(token)) return res.status(400).json({ error: '无效的会话链接' });
    if (!role || !['sender', 'receiver'].includes(role)) return res.status(400).json({ error: '无效的角色' });
    if (!content || typeof content !== 'string' || !content.trim()) return res.status(400).json({ error: '消息内容不能为空' });
    if (content.length > 500) return res.status(400).json({ error: '消息内容不能超过500字' });
    ...
    if (msgCount > 30) return res.status(429).json({ error: '发送消息过于频繁，请稍后再试' });
